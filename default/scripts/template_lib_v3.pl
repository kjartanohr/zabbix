...
# 2023.03.29
  ...
  #foreach ('/usr/share/zabbix/repo/lib/', "$ENV{'HOME}/lib", ""){next if not -f "$_/lib.pm"; eval require "$_/lib.pm"}
...
BEGIN {
  ...
  my $dir_lib           = "$ENV{'HOME'}/perl5/lib/perl5/KFO";
  my $url_zabbix_ext    = "http://zabbix.kjartanohr.no";
  my $url_zabbix_int    = "http://10.0.6.102";
  push @INC, $dir_lib;

  my $url_lib           = "$url_zabbix_int/zabbix/repo/default/scripts/lib-dev.pm";
  print "$url_lib\n";
  system qq#curl -v -k "$url_lib" -o "$dir_lib/lib.pm"#;

  unless (-f "$dir_lib/lib.pm") {
    system qq#mkdir -p $dir_lib# if not -d $dir_lib;
    #system qq#curl -v -k "https://$zabbix_host_int/zabbix/repo/default/scripts/lib-dev.pm" -o "$dir_lib/lib.pm"#;
  }	
}
