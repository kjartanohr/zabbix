...
BEGIN {
  ...
  my $lib_dir           = "$pwd/lib/KFO";
  my $lib_file          = "$lib_dir/lib.pm";
  push @INC, $pwd, $lib_dir, "$pwd/lib";  
  my @lib_dirs          = ('/usr/share/zabbix/repo/lib', "$ENV{'HOME'}/lib", "$ENV{'HOME'}/perl", "$ENV{'HOME'}/perl5", "/tmp/perl/lib", "/tmp/zabbix/perl/lib");
  map {push(@INC, $_) if -d $_} @lib_dirs;
  system "mkdir -p -v $lib_dir" if not -d $lib_dir;

  if (not -f "$lib_file") {
    system qq#curl -v -k "$url_lib" -o "$lib_file"#;
  }	
}

END { close STDOUT } # Stop hanging
...
# init $db
my $json_init                       = $lib->init_json('type' => 'utf8');
my $json                            = $json_init;
