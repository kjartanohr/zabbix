...
if (-f $file_log and -s $file_log > $log_max_size*1024*1024){
  debug("Log file is more than $log_max_size. Trunking the file\n");
  touch($file_log);
}

if (-f $file_debug and -s $file_debug > $log_max_size*1024*1024){
...
while (1) {
  ...
  my $cmd_time_running = (time - $cmd_time);
  my $cmd_time_running_hours = int ($cmd_time_running / 60 / 24);
  $cmd_time = time;
  debug("The command was running for $cmd_time_running_hours hours\n");
  ...
  if (!(-f $file_online) and $cmd_time_running < $cmd_min_run_time) {
    ...
    debug("cmd_error_count: $cmd_error_count. Setting \$cmd_error_count = 0. \n");
    $cmd_error_count = 0;
  }
  ...
  if (-f $file_online) {
    ...
    my $msg_error = "$date HTTP lost connection. Was running for $cmd_time_running_hours hours. Total failed count $cmd_error_count";
