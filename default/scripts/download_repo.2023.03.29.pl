...
foreach (split/\n/, $curl){
  ...
  if ($file_remote_size > $file_size_big) {
    print "Download file size $file_remote $file_remote_size is bigger than $file_size_big. will download this file in the background\n" if $debug;
    $fork = 1;
  }
  else {
    print "Download file size $file_remote $file_remote_size is less than $file_size_big. will not download this file in the background\n" if $debug;
  }
