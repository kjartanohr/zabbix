...
check_cp_software()
{
    ...
    installed_jumbo_version=$(grep ":installed_on " /config/active | grep Bundle | grep -v bundle | grep JUMBO | egrep -o 'T[0-9]{1,3}' | tr -d "T" | sort -n | tail -n1)
    if [[ $current_version -eq "7730" ]]; then
        r7730_ga_jumbo="345"
