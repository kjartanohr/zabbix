...
my $self_name = "KFO::lib 2023-04-04";
...
sub readfile {
  ...
  if (not defined $filename){
    debug("Missing input data 'filename'. return", 'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
    return;
  }
...
my $data = run_cmd({
  ...
  debug("run_cmd() start", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  my @ps_out = run_cmd({
    "cmd"             => 'ps xa',
    'return-type'     => 'a',
    'refresh-time'    => 1,
  ...
  if (not @ps_out){
    debug("\@ps_out has no data. Something is wrong",  'error', \[caller(0)]) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
    warn("\@ps_out has no data. Something is wrong");
    return 0;
  }
  debug("run_cmd() end. \@ps_out: @ps_out", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

  debug("foreach \@ps_out", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  foreach (@ps_out){

    debug("\$line: $_", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 4;


=cut
sub run_cmd {
  my $sub_name = (caller(0))[3];
  $config{'log'}{$sub_name}{'enabled'}  = 1 if $debug;
  $config{'log'}{$sub_name}{'level'}    = 9 if $debug;
  $config{'log'}{'debug'}{'enabled'}    = 1 if $debug;
  $config{'log'}{'debug'}{'level'}      = 9 if $debug;
  debug("start", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  ...
  if (ref $_[0] and $_[0] =~ /HASH/) {
    debug("First data in array is a hash ref", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
    %input = %{$_[0]};
  ...
  unless (defined $input{'cmd'} and $input{'cmd'}) {
    ...
    debug("Missing input data cmd. input: ".Dumper(%input), 'fatal', \[caller(0)]) if $config{'log'}{'fatal'}{'enabled'} and $config{'log'}{'fatal'}{'level'} > 1;
    return;
  }
...
#  }
  ...
  my $message = "$date $self_name $type $name $pre: $msg\n";
...
sub create_dir {
  ...
  if ($config{"val"}{'sub'}{'in'}){
    if (not defined $name and not length $name > 0) {
      debug("missing input data for 'name'", "fatal", \[caller(0)] ) if $config{'log'}{'fatal'}{'enabled'} and $config{'log'}{'fatal'}{'level'} > 3;
      return 0;
    }
  }
  #validate input data start
...
sub check_if_other_self_is_running {
  ...
  unless (defined $input_pid and $input_pid) {
  ...
  if ($name =~ /\(.*?\)/){
    debug("Found extra data in name. Removing data from name", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    $name =~ s/\(.*?\)//;
    debug("new name: $name", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  }


  # remove version from name
  if ($name =~ / VER \d/){
    debug("Found version in name. Removing version from name", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    $name =~ s/ VER.*//;
    debug("new name: $name", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  }
  ...
  foreach (@ps_out){
    ...
    debug("\$line: $_", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 4;
    next unless /$name/;
    debug("Regex match found: $_ =~ /$name/", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    s/^\s{1,}//;
    ...
    debug("Checking if PID from ps is our ($input_pid) or parent PID ($pid)", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    debug("next if $input_pid eq $pid", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    #print("next if $input_pid eq $pid\n");

    next if $input_pid eq $pid;
    next if $$ eq $pid;
    next if $command =~ /PID:$input_pid/;

    next if $command =~ /grep /;

    debug("\$pid: '$pid'. \$tty: '$tty', command: '$command'", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    #print("\$pid: '$pid'. \$tty: '$tty', command: '$command'\n");

    debug("Found other process running. Name: '$name'. Line: '$_'", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    #print("Found other process running. Name: '$name'. Line: '$_'\n");
...
sub init_json {
  ...
  $input{'version'} = 2 if defined $input{'type'} and $input{'type'} eq "utf8";
  ...
  if ($config{"val"}{'sub'}{'in'}){
    ...
    foreach my $input_type (@input_type) {
      ...
      if (not defined $input{$input_type} and not length $input{$input_type} > 0) {
  ...
  if ($input{'version'} == 1){
    ...
    $json->max_size(104_857_600); # 100 MB
...
=pod
  #$json->latin1(0);
  #$json->utf8(0); # keep this off. node-red, home assistant crashes with this on
  $json->pretty(0);
  $json->relaxed(1);
  #$json->canonical(1);
  $json->ascii(1); # If $enable is true (or missing), then the encode method will not generate characters outside the code range 0..127 (which is ASCII). Any Unicode characters outside that range will be escaped using either a single \uXXXX
  $json->pretty(0);
  $json->indent(0);
  $json->space_after(0);
  $json->allow_nonref(1);
  $json->allow_unknown(0);
  $json->allow_nonref->convert_blessed(1);
  $json->allow_tags(1);
  #$json->get_canonical(1); # sort output
  #$json->get_convert_blessed(1);
  #$json->get_max_depth(100);
  #$json->get_max_size(4000);
=cut

    $return = $json;
  }
  else {
    debug("wrong input version. \$input{'version'}: '$input{'version'}'", "fatal", \[caller(0)] ) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
  }
...
sub json_to_hash {
  ...
  $input{'json_string'}  //= $input{'json'}   if defined $input{'json'};
  $input{'json_string'}  //= $input{'string'} if defined $input{'string'};

  #validate input data start
  if ($config{"val"}{'sub'}{'in'}){
    my @input_type = qw( json_string );
    foreach my $input_type (@input_type) {
  ...
  if (not defined $input{'json_string'}){
    debug("missing input data for 'json_string'", "fatal", \[caller(0)] ) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 3;
    return 0;
...
=pod
  my $filename_safe = get_filename_safe( 'string' => 'filename !! \\');
  my $filename_safe = get_filename_safe( 'string' => 'filename !! \\', 'exit-if-fatal' => 1);
...
sub get_filename_safe {
  ...
  $input{'string'} //= $input{'name'} if defined $input{'name'};


  my @input_type = qw( string );
  foreach my $input_type (@input_type) {
...
#read_fh START
my $read_fh_data = {};
my $read_fh_status = read_fh(
  'fh'        => $fh_r,
  'hash_ref'  => $read_fh_data,
  'desc'      => "short desc. Line: ".\[caller(2)],
  'config'    => \%config,
);

if ($read_fh_status) {
  debug("read_fh_status() status is OK. Data returned: ".Dumper($read_fh_data), "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
}
else {
  debug("read_fh() status FAILED. ".Dumper($read_fh_data)", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  debug("read_fh() status FAILED. ".Dumper($read_fh_data)", 'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
  #return 0;
}

foreach my $line (@{$input{'hash_ref'}{'line'}}){
  debug("\$line: $line", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 4;
...
sub read_fh {
  ...
  debug("start", "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 2;
  debug("Input: ".Dumper(@_), "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 3;
  ...
  #Validate input data START
  if ($config{"val"}{'sub'}{'in'}){
    my @input_type = qw( hash_ref fh desc config );
    foreach my $input_type (@input_type) {
  ...
  debug("my \$s = IO::Select->new;", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
  my $s = IO::Select->new;

  debug("\$s->add(\$input{'fh'});", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
  $s->add($input{'fh'});

  debug("my \@data_read = \$s->can_read(1); #blocking N sec", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
  my @data_read = $s->can_read($input{'timeout'}); #blocking N sec

  debug("validating \@data_read", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
  unless (@data_read) {
    debug("No data from \$s->can_read(1); return 0", "debug", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 0;
    $input{'hash_ref'}{'message'} = "No data from \$s->can_read(1); return 0";
    return 0;
  }
  debug("Data found in \@data_read: ".scalar @data_read, "debug", \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;

  debug("DATA_READ: foreach my \$data_read \@data_read", "debug", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
  DATA_READ:
  foreach my $data_read (@data_read) {
    debug("\$data_read: '$data_read'", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;

    unless (defined $data_read) {
      my $debug_msg = "\$data_read is not defined. something is wrong";
      debug($debug_msg,  'error', \[caller(0)]) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
      push @{$input{'hash_ref'}{'log'}{'error'}}, $debug_msg;
    }

    if (eof $data_read) {
      my $debug_msg = "\$data_read is eof. something is wrong";
      debug($debug_msg,  'error', \[caller(0)]) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
      push @{$input{'hash_ref'}{'log'}{'error'}}, $debug_msg;
    }

    my $readline = readline($data_read);
    unless (defined $readline) {
      my $debug_msg = "\$readline is not defined. something is wrong";
      debug($debug_msg,  'error', \[caller(0)]) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
      push @{$input{'hash_ref'}{'log'}{'error'}}, $debug_msg;
    }
  
    debug("\$readline: '$readline'",  $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

    push @{$input{'hash_ref'}{'line'}}, readline($data_read);
    $return = 1;
  }
  ...
  if ($config{"val"}{'sub'}{'out'}){
    ...
    if ($input{'validate-return-data'} and defined $return and length $return == 0) {
      my $debug_msg = "Return data is not defined. Input data: ".Dumper(%input);
      debug($debug_msg,  'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
      push @{$input{'hash_ref'}{'log'}{'error'}}, $debug_msg;
  ...
  debug("return data: ".Dumper($input{'hash_ref'}), "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 4;
...
sub string_rewrite {
  ...
  #validate input data start
  if ($config{"val"}{'sub'}{'in'}){
    my @input_type = qw( string template );
    foreach my $input_type (@input_type) {
  ...
  unless (defined $$rewrite_template{$input{'template'}}) {
    debug("Could not find the template for: $input{'template'}", 'fatal', \[caller(0)]) if $config{'log'}{'fatal'}{'enabled'} and $config{'log'}{'fatal'}{'level'} > 1;
  }


  while (my ($regex_from, $regex_to) = each %{$$rewrite_template{$input{'template'}}}) {
    debug("regex from: '$regex_from'. regex to: '$regex_to'. String: ${$input{'string'}}", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
    
    ${$input{'string'}} =~ s/$regex_from/$regex_to/g;
    
  }
  debug("String after rewrite: ${$input{'string'}}", 'debug', \[caller(0)]) if $config{'log'}{'debug'}{'enabled'} and $config{'log'}{'debug'}{'level'} > 1;
...
sub get_local_id {
  ...
  foreach my $cmd (@cmd) {
    debug("\$cmd: $cmd", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    my $cmd_out = run_cmd({
      'cmd'             => $cmd,
      'return-type'     => 's', 
      #'vsid'            => 0, 
      'refresh-time'    => 1*24*60*60,
      'timeout'         => 5,
      'include-stderr'  => 1,
      'desc'            => "$sub_name. create local id $cmd",
    });

    chomp $cmd_out;
    if ($cmd_out =~ /command not found/){
      debug("command not found: $cmd",  'error', \[caller(0)]) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
      next;
    }

    $cmd_out = get_filename_safe('string' => $cmd_out);
    debug("\$cmd_out after rewrite: $cmd_out", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
...
2023-04-19 10:44:27
@todo whereis
Legge til enkel sjekk av /bin, /usr/bin osv før kommandoen whereis kjøres
...
sub whereis {
  ...
  if (not @_){
    debug("input data missing. No data found in \@_", 'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
    return;
  }
  ...
  foreach my $file (@files) {
    debug("Looking for file $file", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    ...
    my $out = run_cmd({
      ...
      'timeout'         => 10,
      'include-stderr'  => 1,
      'desc'            => "$sub_name. whereis $file",
    });

    chomp $out;
    debug("\$out: $out", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

    my ($path) = $out =~ /: (.*)/;

    if (not defined $path){
      debug("\$path is not defined. next", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
      next;
    }

    $path =~ s/ .*//;
    debug("\$path after rewrite", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

    if ($path) {
      debug("Found file: '$file'. path: '$path'", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
      return $path;
    }
  }
    
  debug("Could not find file with whereis", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  ...
  foreach my $file (@files) {
    ...
    my @find_out = run_cmd({
      ...
      'timeout'         => 600,
      'include-stderr'  => 0,
      'desc'            => "$sub_name. find / -name $file",
    });


    foreach my $find_file (@find_out) {
      next if not defined $find_file;
      chomp $find_file;
      next if not $find_file;

      debug("Found file $file. $find_file", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
      return $find_file;
    }
  }
...
sub check_dns_resolve {
  ...
  if (not defined $hostname){
    debug("\$hostname is not defined. return",  'error', \[caller(0)]) if $config{'log'}{'error'}{'enabled'} and $config{'log'}{'error'}{'level'} > 1;
    return;
  }
  debug("\$hostname: '$hostname'", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

  my $cmd_dig     = "dig +timeout=2 $hostname";
  my $cmd_dig_out = run_cmd({
    'cmd'             => $cmd_dig,
    'return-type'     => 's', 
    #'vsid'            => 0, 
    'refresh-time'    => 1*60*60,
    'timeout'         => 10,
    'include-stderr'  => 1,
    'desc'            => "$sub_name. $cmd_dig. Check if dns works",
  });
  debug("\$cmd_dig_out: '$cmd_dig_out'", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

  debug("Looking for: status: NOERROR", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  if ($cmd_dig_out =~ /status: NOERROR/) {
    debug("Resolve OK\nOutput: $cmd_dig_out", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    $return = 1;
  }
  else {
    debug("Resolve FAILED. Output: $cmd_dig_out", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    $return = 0;
  }
...
my $kill_old_version_running_status = kill_old_version_running(
  'name'      => $0,
  'pid'       => $$,
  'version'   => $version,
);
...
sub kill_old_version_running {
  ...
  #validate input data start
  if ($config{"val"}{'sub'}{'in'}){
    my @input_type = qw( name pid version );
    foreach my $input_type (@input_type) {
  ...
  if ($input{'name'} =~ /\(.*?\)/){
    debug("Found extra data in name. Removing data from name", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    $input{'name'} =~ s/\(.*?\)//;
    debug("new name: $input{'name'}", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
  }
  ...
  foreach (@ps_out){
    ...
    debug("ps xa: found $input{'name'}: $_", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

    #PID TTY      STAT   TIME COMMAND
    my ($pid,$tty,$stat,$time,$command) = split /\s{1,}/, $_, 5;

    debug("next if $input{'pid'} eq $pid;", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    next if $input{'pid'} eq $pid;
    next if $$ eq $pid;
    next if $command =~ /PID:$input{'pid'}/;
    #next if /grep /;
    ...
    if (not defined $ver) {
      ...
      warn("\$ver not defined. Could not find version in process name: $_");
      next;
    }
    chomp $ver;
    debug("\$ver: '$ver'", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    ...
    if ($ver ne $input{'version'}){
      debug("Found a different version of the script Will kill it: $pid", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
      print("Found a different version of the script Will kill it. $ver ne $input{'version'} $pid\n");
      kill_pid('pid' => $pid);
      $return = 1;
    }
    else {
      debug("Found the same version of the script", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
      print("Found the same version of the script. $ver eq $input{'version'} $pid\n");
      #return 0;
    }
...
sub kill_pid {
  ...
  my $process_alive = kill  0, $input{'pid'};
  unless ($process_alive) {
    debug("The PID/process is not alive. Will not try to kill it. PID: $input{'pid'}", "debug", 'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
    return 0;
  }


  debug("Process is alive: PID: $input{'pid'}", "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

  my $kill_retry_max_count  = 10;

  KILL:
  foreach my $try (1 .. $kill_retry_max_count) {
    debug("kill try: $try", "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;

    debug("checking if the PID is alive. kill 0, $input{'pid'}", "debug", $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    my $pid_alive = kill 0, $input{'pid'};

    unless ($pid_alive) {
      debug("The PID/process is not alive. return 1", "debug", 'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
      return 1;
    }

    #my $sigkill_status = kill 'SIGKILL', $input{'pid'};
    #debug("SIGKILL returned status true",   'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1 and $sigkill_status;
    #debug("SIGKILL returned status false",  'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1 and not $sigkill_status;
    kill    'SIGKILL',  $input{'pid'}   if kill 0, $input{'pid'};
    kill    'KILL',     $input{'pid'}   if kill 0, $input{'pid'};
    kill    'TERM',     $input{'pid'}   if kill 0, $input{'pid'};
    system  "kill -9    $input{'pid'}"  if kill 0, $input{'pid'};

    #Check if the PID is alive
    $process_alive = kill 0, $input{'pid'};
    unless ($process_alive) {
      debug("The PID/process is not alive. last KILL", 'warning', \[caller(0)]) if $config{'log'}{'warning'}{'enabled'} and $config{'log'}{'warning'}{'level'} > 1;
      last KILL;
    }
  }
