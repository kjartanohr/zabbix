...
#LOG
#2022.12.06 - bug in parse_command_line
...
sub parse_command_line {
  ...
  if ($input{'argv'}[0] =~ /--/){
    debug("if (\$input{'argv'}[0]} =~ /--/ is true",  $sub_name, \[caller(0)]) if $config{'log'}{$sub_name}{'enabled'} and $config{'log'}{$sub_name}{'level'} > 1;
    @{$input{'argv'}} = $input{'argv'}[0] =~ /(--.*?")\s{1,}/g;
  }
...
sub get_config {
  ...
  #Not sure what this is. 20202.08.11
  $config{'log'}{'all'}       = {
    "enabled"       => 0,     #0/1
    "level"         => 1,     #0-9
